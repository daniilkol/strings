...
int split_words(const char* s,char** words)
{
    ...
    int i,f=1,j=0,pos,dlina_s=strlen(s);
    for (i=0;i<=dlina_s;i++)
    {
        if(!(s[i]==0))
        {
...
{
    int i,kolichestvo=0,dlina_s=strlen(s),p,j,f=0,k,h;
    FILE *fp1;
    FILE *fp2;
   ...
   if(!(wordbuf=(char**)malloc(LEN*sizeof(char*))))
   {
       printf("Not enough memory\n");
       return 2;
   }
    ...
    while (fgets(buf,LEN,fp1))
    {
        ...
        h=split_words(buf,wordbuf);
        for(i=0;i<=j;i++)
        {
            for(k=0;k<=h;k++)
            {
                if (strcmp(wordbuf[k],words[i])==0)
                {
                kolichestvo++;
